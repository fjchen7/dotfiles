return {
  ...
  config = function(_, opts)
    ...
    map({ "n", "x" }, "[c", move("comment", "backward"), "prev comment")
    map("o", "[c", move("comment", "backward", { wrap = false }), "prev comment")
    map({ "n", "x" }, "]c", move("comment", "forward"), "next comment")
    map("o", "]c", move("comment", "forward", { wrap = false }), "prev comment")
    -- Conflict
    ...
    map({ "n", "x", "o" }, "]I", move("indent", "last"), "outermost indent end")
    map({ "n", "x", "o" }, "[I", move("indent", "first"), "outermost indent start")
  end
}
