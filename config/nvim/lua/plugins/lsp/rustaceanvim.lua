  ...
  opts = {
    ...
    server = {
      on_attach = function(client, bufnr)
        ...
        Snacks.toggle({
          name = "Rust Analyzer checkOnSave",
          get = function()
            local rust_lsp = vim.lsp.get_clients({ name = "rust-analyzer" })[1]
            local ra_settings = rust_lsp.config.settings["rust-analyzer"]
            return ra_settings.checkOnSave
          end,
          set = function(state)
            local rust_lsp = vim.lsp.get_clients({ name = "rust-analyzer" })[1]
            local ra_settings = rust_lsp.config.settings["rust-analyzer"]
            ra_settings.checkOnSave = state
            rust_lsp.notify("workspace/didChangeConfiguration", { settings = ra_settings })
          end,
        }):map("<leader>cs", { buffer = bufnr })
      end,
