...
set("n", "f", [[sneak#is_sneaking() ? '<Plug>Sneak_;' : '<Plug>Sneak_f']], opts)
set("n", "F", [[sneak#is_sneaking() ? '<Plug>Sneak_,' : '<Plug>Sneak_F']], opts)
...
-- end
set("", "n",
  [[sneak#is_sneaking() ? '<Plug>Sneak_;' : v:searchforward ? 'n' : 'N''<cmd>lua require("hlslens").start()<cr>']], opts)
set("", "N",
  [[sneak#is_sneaking() ? '<Plug>Sneak_;' : v:searchforward ? 'N' : 'n''<cmd>lua require("hlslens").start()<cr>']], opts)

-- set("n", "<Esc>", [[sneak#is_sneaking() ? ':<C-u>call sneak#cancel()<cr>' : '<Esc><cmd>noh<cr>']], opts)
set("v", "<Esc>", [[sneak#is_sneaking() ? '<Esc>:<C-u>call sneak#cancel()<cr>' : '<Esc>']], opts)
set("n", "<Esc>", function()
