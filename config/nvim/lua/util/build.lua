...
function M.keymaps()
  ...
  lazy_util.foreach(core.plugins, function(name, plugin)
...
function M.update()
  ...
  lazy_util.walk(root .. "/lua/plugins/extras", function(path, name, type)
...
```
  ...
  data.examples = lazy_util.read_file(examples):gsub("^[^\n]+\n[^\n]+\n[^\n]+\n", "")
  Docs.save(data)
end
